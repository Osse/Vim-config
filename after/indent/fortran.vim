...
if (b:fortran_fixed_source != 1)
	...
	setlocal indentkeys+==~subroutine,=~function,=~forall,=~module,=~program
	setlocal indentkeys+==~endsubroutine,=~endfunction,=~endforall,=~endmodule,=~endprogram
...
function SebuFortranGetFreeIndent()
	...
	if line =~? '^\s*\(pure\|elemental\)\=\s*\(subroutine\|function\|module\)' ||
				\ line =~? '^\s*\(forall\)'
		let ind += &sw
	endif
	if getline(v:lnum) =~? '^\s*end\s*\(subroutine\|function\|forall\|module\)'
		let ind -= &sw
	endif
